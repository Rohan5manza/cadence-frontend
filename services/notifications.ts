...
// ── Daily reminder — 8pm if user hasn't read ─────────────────────────────────
export async function scheduleDailyReminder(): Promise<void> {
  ...
  const morningMessages = [
    ...
    { title: '🧠 Brain empty?',               body: "We have million of papers to fix that. Pick one." },
...
export async function sendStreakBrokenNotification(lostStreak: number): Promise<void> {
  const messages = [
    ...
    { title: '🪦 RIP your streak.',            body: `${lostStreak} days, 2024-2025. Gone too soon. Start a new one today and don't let it happen again.` },
